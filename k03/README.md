...
## ソースコードの説明
　arg[0]には./k03、arg[1]には170.8、arg[2]には、、、と実行時に入力したものが格納されているので、sscanfでmu,sigma,num_dummyにそれぞれ読み取る。
　捏造するのが5人なので、for文の継続条件を5以下にすることで5人分のデータを捏造できる。捏造する関数は dummy = ((r_stdnorm-0)/1)*sigma + muとなる。
## 入出力結果
