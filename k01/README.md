...
## ソースコードの説明
最初に公式を使って平均を出す関数と分散を出す関数を作る。
while文でデータを一つずつ増やしながら平均と分散を出す。平均よりも分散を先にやった理由は先に平均をやってしまうと、分散の公式内での平均が一つずれてしまうためである。
不偏分散を分散を使って出す。N++を平均や分散の前にやったため平均と分散のときのNと同じ値を使える。後にするとNが1増える。
...
- 出力結果が足りません
  - 標本平均、標本分散、**母集団平均と誤差**、母集団分散の4つを出力してください。
  - 誤差の計算が必要になります。その他の計算は大丈夫です。
